...
def plot_sherical_harmonics(file_name="test",
    ...
    os.system(f'gmt gmtset FORMAT_GEO_MAP ddd',     # Set the format of the map
               'set ascii_file={file_name}.txt',    # Set the ascii file
               'set grid_file={file_name}.grd',     # Set the grid file
               'gmt begin {file_name} {img_type}',  # Start the plot
               'gmt xyz2grd %ascii_file% -R{region} -r -I{grid_resolution} -G%grid_file% -V',           # Convert the ascii file to a grid file
               'gmt grd2cpt %grid_file% -C{color_palette} -Z',                                          # Create the color palette for the grid file
               'gmt grdimage -J{map_projection} -R{region} %grid_file% -Q',                             # Plot the grid file                    
               'gmt psxy {file_name_poly} -W3,red',                                                     # Plot region polygon
               'gmt coast -Bxa5g5 -Bya5g5 -BWESN+t"{title}" -W0.25p,80/80/80 -Df -N1/1.25p,black -V',   # Plot the coastline and the title
               'gmt text -F+cBL+t"{subtitle}" -N -D6.65c/-1c',  # Plot the subtitle
               'gmt text -F+cBL+t"{editors}" -N -D5.15c/-1.5c', # Plot the editors
               'gmt colorbar {colorbar_settings}'               # Plot the colorbar
               'gmt end')
