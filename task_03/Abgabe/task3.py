...
def run_gmt(input_file_name="test_input",
    ...
    if(print_vector is True):
        ...
        command += f' && gmt grdvector ./data/{vector_1}.nc ./data/{vector_2}.nc -WRed -Ix2 -S0.5 -Q0.2+e'              # Plot the vectors
    else:
        command += f' && gmt grdimage -J{map_projection} -R{region} ./data/{output_file_name}.nc -Q'                    # Plot the grid
    ...
    for index_phi, phi in enumerate(lat):
        ...
        for index_lam, lam in enumerate(lon):
            ...
            if index_lam == 0:
                ...
                mag[index_phi][index_lam] = np.sqrt(u[index_phi][index_lam]**2 + v[index_phi][index_lam]**2)
    return u, v, mag
...
if __name__ == '__main__':
    ...
    u, v, magnitude = calc_velocity(mdt, lon, lat, grid_spacing=0.5)
    ...
    fu.save_grid(os.path.join(f'./data/DTU15MDT_30min_velocity.mdt.nc'), magnitude, lon, lat)
    ...
    run_gmt(input_file_name="DTU15MDT_30min_velocity.mdt",
            ...
            editors="Editors: Christopher Mahn, Silas Teske, Joshua Wolf",
            colorbar_settings='-Dx0c/-2c+w17c/0.35c+h -B0.5+l"Velocity [m/s]" -V'
            )
    ...
    run_gmt(input_file_name="DTU15MDT_30min_cut_pyout.mdt",
            ...
            subtitle="Mean dynamic ocean topography (MDT) with geostrophic velocity vectors",
            editors="Editors: Christopher Mahn, Silas Teske, Joshua Wolf",
            colorbar_settings='-Dx0c/-2c+w17c/0.35c+h -B0.2+l"MDT [m]" -V'
            )
